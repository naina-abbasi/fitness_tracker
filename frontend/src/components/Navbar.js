 ...
 *
 * Props:
 *  - onToggleTheme: () => void   — callback to flip dark/light mode
 *  - isDarkMode: boolean          — current theme state
 *  - user: { name, avatar? }      — optional logged-in user info
 */
function Navbar({ onToggleTheme, isDarkMode, user }) {
  ...
  return (
    <AppBar
      ...
      sx={{
        ...
        borderBottom: `1px solid ${
          isDarkMode ? alpha("#7c6af7", 0.18) : alpha("#7c6af7", 0.12)
        }`,
    ...
    >
      <Toolbar sx={{ px: { xs: 2, sm: 3 }, minHeight: { xs: 64, sm: 70 } }}>
        ...
        {/* ── Streak chip (decorative) ── */}
        <Chip
          icon={
            <LocalFireDepartmentIcon
              sx={{ fontSize: "14px !important", color: "#ff6b35 !important" }}
            />
          }
          label="7-day streak"
          size="small"
          ...
          PaperProps={{
            ...
            sx: {
              ...
              boxShadow: isDarkMode
                ...
                : "0 20px 60px rgba(124,106,247,0.15)",
              overflow: "visible",
              "&::before": {
                content: '""',
                display: "block",
        ...
        >
          ...
          <Box sx={{ px: 2, py: 1.5 }}>
            ...
            <Typography
              variant="caption"
              sx={{
                color: isDarkMode ? alpha("#fff", 0.45) : alpha("#000", 0.45),
              }}
            >
              {user?.email || "fitness@example.com"}
            </Typography>
          </Box>
